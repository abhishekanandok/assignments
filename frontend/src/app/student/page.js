...
export default function StudentPage() {
  ...
  const fetchSubmissionById = async (submissionId) => {
    try {
      const res = await fetch(`http://localhost:5000/api/student/submission/${submissionId}`, {
        headers: { "Authorization": `Bearer ${token}` }
      });
      const data = await res.json();
      console.log("API Response:", data);
      if (data.success && data.submission) {
        console.log("Setting result:", data.submission.result);
          ...
          <TabsContent value="new">
            {!result ? (
              ...
              <div className="text-center py-12">
                ...
                <Button variant="outline" size="sm" onClick={() => { setResult(null); setActiveTab("history"); }} className="mt-4">
                  ← Back to History
                </Button>
              </div>
            ) : (
          ...
          <TabsContent value="history" className="animate-fade-in">
            <Card className="shadow-sm border-0 bg-white/80">
              <CardHeader>
                ...
                <CardDescription>View your past assignment evaluations and scores. Results appear after teacher approval.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                {historySubmissions.length === 0 ? (
                  ...
                  historySubmissions.map(sub => (
                    <div key={sub._id} className={`border border-border/50 rounded-xl p-4 transition-colors ${sub.publishedToStudent ? 'bg-muted/30 hover:bg-muted/50' : 'bg-amber-50/50 border-amber-200'}`}>
                          ...
                          {!sub.publishedToStudent && (
                            <div className="flex items-center gap-1 mt-1">
                              <Badge variant="warning" className="text-xs">
                                <Clock className="w-3 h-3 mr-1" /> Pending Teacher Review
                              </Badge>
                            </div>
                          )}
                          {sub.publishedToStudent && sub.teacherFeedback && (
                            <p className="text-xs text-blue-600 mt-1 italic">&ldquo;{sub.teacherFeedback}&rdquo;</p>
                          )}
                        </div>
                      ...
                      {sub.publishedToStudent ? (
                        <Button variant="outline" size="sm" className="w-full text-xs" onClick={() => fetchSubmissionById(sub._id)}>
                        ...
                        <div className="flex items-center gap-2 text-xs text-amber-700 bg-amber-100 rounded-lg px-3 py-2">
                          <AlertCircle className="w-4 h-4 flex-shrink-0" />
                          Your submission is being reviewed by your teacher. Results will be available after approval.
                        </div>
                      )}
                    </div>
                  ))
